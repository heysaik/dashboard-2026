...
@MainActor final class WidgetGenerator {
    ...
    func generate(prompt: String, provider: String, endpoint: String, model: String, widget: Bool = true, theme: String = "leopard", research: Bool = false, size: String = "medium") async throws -> String {
        ...
        if provider == "claude" {
            arguments = ["--print", "--safe-mode", "--tools", widget || research ? "WebSearch,WebFetch" : "", "--strict-mcp-config", "--no-session-persistence", "--output-format", "json", "--system-prompt", system]
            if widget || research { arguments += ["--allowedTools", "WebSearch,WebFetch", "--permission-mode", "dontAsk"] }
            if widget { arguments += ["--json-schema", Self.schema] }
        } else {
            arguments = (widget || research ? ["--search"] : []) + ["exec", "--ignore-user-config", "--skip-git-repo-check", "--ephemeral", "--sandbox", "read-only", "-c", "features.shell_tool=false", "--color", "never", "--output-last-message", output.path]
            if widget {
