...
const Widgets = (() => {
  ...
  const thumbnail = inner => `<svg class="classic-thumbnail" viewBox="0 0 64 64" fill="none" aria-hidden="true">${inner}</svg>`;
  const clockIcon = thumbnail(`<defs><radialGradient id="shelf-clock"><stop stop-color="#fffef7"/><stop offset="1" stop-color="#dfdfd5"/></radialGradient></defs><circle cx="32" cy="32" r="30" fill="#333" stroke="#777"/><circle cx="32" cy="32" r="26.5" fill="url(#shelf-clock)" stroke="#101010"/>${Array.from({length:60},(_,i)=>`<path d="M32 7v${i%5===0?3:1.3}" stroke="#454545" stroke-width="${i%5===0?.8:.4}" transform="rotate(${i*6} 32 32)"/>`).join('')}${Array.from({length:12},(_,i)=>{const n=i+1,a=n*Math.PI/6;return `<text x="${32+18.8*Math.sin(a)}" y="${34.2-18.8*Math.cos(a)}" text-anchor="middle" fill="#252525" font-family="Helvetica" font-size="6.4">${n}</text>`}).join('')}<path d="M32 32 23 23M32 32l14-14" stroke="#242424" stroke-width="2" stroke-linecap="round"/><path d="M32 36V12" stroke="#c23329" stroke-width=".7"/><circle cx="32" cy="32" r="1.8" fill="#333"/>`);
  const calcKeys = ['7','8','9','×','4','5','6','−','1','2','3','+','0','.','c','='];
  const calcIcon = thumbnail(`<defs><linearGradient id="shelf-calc" x2="0" y2="1"><stop stop-color="#f8c781"/><stop offset="1" stop-color="#b57732"/></linearGradient></defs><rect x="7" y="1" width="50" height="62" rx="5" fill="url(#shelf-calc)" stroke="#92632f"/><rect x="11" y="5" width="42" height="12" rx="1" fill="#bce7ec" stroke="#b78e58"/><text x="50" y="14" text-anchor="end" font-family="Helvetica" font-size="9" fill="#285461">0</text>${['m+','m−','mc','mr'].map((n,i)=>`<circle cx="${15+i*11}" cy="23" r="4" fill="#e9e9e5" stroke="#c0c0ba" stroke-width=".5"/><text x="${15+i*11}" y="24.2" text-anchor="middle" fill="#535353" font-family="Helvetica" font-size="3">${n}</text>`).join('')}${calcKeys.map((n,i)=>`<circle cx="${15+i%4*11}" cy="${33+Math.floor(i/4)*8.2}" r="4" fill="#f5f5f0" stroke="#b4b4ae" stroke-width=".5"/><text x="${15+i%4*11}" y="${35+Math.floor(i/4)*8.2}" text-anchor="middle" fill="#343b3c" font-family="Helvetica" font-size="5.6">${n}</text>`).join('')}`);
  const calIcon = () => `<div class="icon-calendar"><small>${new Date().toLocaleDateString('en-US', { weekday: 'long' })}</small>${new Date().getDate()}</div>`;
  const noteIcon = thumbnail('<defs><linearGradient id="shelf-paper" x2="0" y2="1"><stop stop-color="#fffcb1"/><stop offset="1" stop-color="#f3e985"/></linearGradient></defs><path d="M4 3h56v48L50 61H4Z" fill="url(#shelf-paper)" stroke="#bbb467"/><path d="M50 61V51h10" fill="#e4d769" stroke="#bbb467"/><path d="M6 6h51" stroke="#ffffffaa"/>');
  ...
  function weatherArt(code, day = true) { if (code == null) return ''; if (!day && code <= 2) return '<div class="moon"></div>'; if (code === 0 || code === 1) return '<div class="sun"></div>'; if (code === 2) return '<div class="sun"></div><div class="cloud"></div>'; return `<div class="cloud"></div>${code >= 51 ? '<span style="position:absolute;top:28px;left:18px;color:#cbe7ff;font-size:20px">╱ ╱ ╱</span>' : ''}`; }
  ...
  function locationSettings(prefs) { return `${input('city', 'City', prefs.city)}<button type="button" class="silver-button find-city">Find City</button><div class="location-results"></div>${select('unit', 'Temperature', ['Fahrenheit', 'Celsius'], prefs.unit)}${'showLows' in prefs ? `<label class="checkbox"><input type="checkbox" data-setting="showLows" ${prefs.showLows ? 'checked' : ''}>Show forecast lows</label>` : ''}${'showLows' in prefs ? `${select('currentSource','Current temperature',['Nearby observation','Forecast model'],prefs.currentSource || 'Nearby observation')}${prefs.lastWeatherSource ? `<p>${e(prefs.lastWeatherSource)}</p>` : ''}` : ''}<p>Forecasts: Open-Meteo model. Current US temperatures prefer nearby NWS observations when available.</p>`; }

  register('weather', 'Weather', 246, 168, weatherIcon, ctx => {
    ...
    const fit = animated => {
      ...
      ctx.resize(liquid ? (expanded ? 348 : 170) : 246, liquid ? 170 : expanded ? (ctx.prefs.showLows ? 182 : 168) : 108, animated);
    };
    const bind = () => {
      const toggle = ctx.el.querySelector('.weather-top');
    ...
    const load = async () => {
      try {
        const data = await Dashboard.native('weather',ctx.prefs); if (!ctx.alive()) return;
        if (!data.current || !data.daily) throw new Error('Forecast is unavailable for this location.');
        const source=data.source, observed=source.kind==='observation', updated=new Date(source.timestamp*1000).toLocaleTimeString([],{timeZone:data.timezone,hour:'numeric',minute:'2-digit'});
        const condition=data.current.description || Core.weatherDescription(data.current.weather_code,!!data.current.is_day);
        ctx.prefs.lastWeatherSource=`${observed?'Observed':'Model estimate'}: ${source.name}${source.distanceKm!=null?' · '+source.distanceKm.toFixed(1)+' km from selected city':''} · ${updated}. Forecasts: Open-Meteo.`;ctx.save();
        ctx.el.innerHTML = `<div class="weather-face"><div class="weather-top"><div class="weather-place">${e(ctx.prefs.city)}<div class="weather-hi-lo">H: ${Math.round(data.daily.temperature_2m_max[0])}°<span class="classic-break"><br></span> L: ${Math.round(data.daily.temperature_2m_min[0])}°</div></div><div class="weather-art">${weatherArt(data.current.weather_code, data.current.is_day)}</div><div class="weather-condition modern-only ${observed?'observed':''}" title="${e(condition)}">${e(observed && condition==='Observed temperature'?'Observed':condition)}</div><div class="weather-temp">${Math.round(data.current.temperature_2m)}°</div></div><div class="forecast">${data.daily.time.slice(0, 6).map((date, index) => `<div class="forecast-day"><div class="day">${new Date(date*1000).toLocaleDateString('en-US', {timeZone:data.timezone,weekday:'short'}).toUpperCase()}</div>${miniWeather(data.daily.weather_code[index])}<div class="temp">${Math.round(data.daily.temperature_2m_max[index])}°${ctx.prefs.showLows ? `<small class="forecast-low">${Math.round(data.daily.temperature_2m_min[index])}°</small>` : ''}</div></div>`).join('')}</div><button class="weather-source" title="${e(ctx.prefs.lastWeatherSource)}">${observed?'Observed':'Model'} · ${e(updated)}</button></div>`;
        ctx.el.querySelector('.weather-source').onclick=()=>ctx.flip();
        bind();ctx.el.title=ctx.prefs.lastWeatherSource;
  ...
  register('tilegame', 'Tile Game', 158, 180, box(svg('<path d="M5 5h54v54H5z" fill="#b6dc67"/><path d="M5 5h18v18H5zM23 23h18v18H23zM41 41h18v18H41z" fill="#689e31"/><path d="M23 5v54M41 5v54M5 23h54M5 41h54" stroke="#365121"/>')), ctx => {
    ...
    const render = () => {
      ctx.el.innerHTML = `<div class="tile-face"><div class="tile-board ${p.picture === 'Numbers' ? '' : 'photo-tiles'}">${p.board.map((value, index) => `<button data-index="${index}" data-value="${value}" style="background-position:${value % 4 * 100 / 3}% ${Math.floor(value / 4) * 100 / 3}%" class="${value === 15 ? 'hole' : ''}" aria-label="${value === 15 ? 'Empty tile' : 'Tile ' + (value + 1)}">${value === 15 ? '' : value + 1}</button>`).join('')}</div><div class="tile-status">${p.board.every((v, i) => v === i) ? 'Solved!' : `${p.moves || 0} moves · click a tile`}</div></div>`;
      ctx.el.querySelectorAll('[data-index]').forEach(button => button.onclick = () => { const index = Number(button.dataset.index), hole = p.board.indexOf(15); if (Math.abs(index % 4 - hole % 4) + Math.abs(Math.floor(index / 4) - Math.floor(hole / 4)) !== 1) return; const value = p.board[index], oldRect = button.getBoundingClientRect(); [p.board[index], p.board[hole]] = [p.board[hole], p.board[index]]; p.moves = (p.moves || 0) + 1; render(); const moved = ctx.el.querySelector(`[data-value="${value}"]`), newRect = moved.getBoundingClientRect(), scale = Dashboard.state.settings.scale; Dashboard.animate(moved, [{ transform: `translate(${(oldRect.x-newRect.x)/scale}px,${(oldRect.y-newRect.y)/scale}px)` }, { transform: 'translate(0,0)' }], { duration: 170 }); moved.focus({ preventScroll: true }); ctx.save(); });
    }; render();
  ...
  register('flight', 'Flight Tracker', 263, 205, box('✈'), ctx => {
    ctx.el.innerHTML = `<div class="flight-face"><div class="flight-plane">${Icons.symbol('flight')}</div><h2>Flight Tracker</h2><form><input aria-label="Airline and flight number" placeholder="Airline + flight number, e.g. UAL 238" required><button class="silver-button">Track Flight</button></form><p>Opens current flight details on FlightAware.<br>The original Dashboard flight feed is retired.</p></div>`;
  ...
  register('sports', 'ESPN', 255, 278, box('ESPN', 'red'), ctx => {
    ...
    let request = 0;
    const load = async () => { const id = ++request; try { const data = await fetchJSON(`https://site.api.espn.com/apis/site/v2/sports/${leagues[p.league]}/scoreboard`); if (!ctx.alive() || request !== id) return; if(!Array.isArray(data.events))throw new Error('Scoreboard unavailable: the source returned incomplete data.'); ctx.el.querySelector('.sports-results').innerHTML = data.events?.length ? data.events.map(event => `<div class="score-game"><div class="score-status">${e(event.status?.type?.shortDetail || new Date(event.date).toLocaleString())}</div>${(event.competitions?.[0]?.competitors || []).map(team => `<div class="score-team"><span>${e(team.team?.abbreviation || team.team?.displayName)}</span><b>${e(team.score ?? '—')}</b></div>`).join('')}</div>`).join('') : '<div class="service-error">No games scheduled today.</div>'; } catch (error) { if (ctx.alive() && request === id) ctx.el.querySelector('.sports-results').innerHTML = `<div class="service-error">${e(error.message)}</div>`; } };
  ...
  register('ski', 'Ski Report', 232, 242, box('❄', 'wood'), ctx => {
    ctx.el.innerHTML = `<div class="ski-face"><h2>${e(ctx.prefs.city)}</h2><div class="ski-temp">—°</div><div class="ski-data">Loading mountain weather…</div><div class="ski-note">Mountain forecast · Open-Meteo<br>Lift and trail reports are not available.</div></div>`;
    const load = async () => { try { const data = await fetchJSON(weatherURL(ctx.prefs, ',snowfall_sum')); if (!ctx.alive()) return; const snow=data.daily?.snowfall_sum; if(!Number.isFinite(data.current?.temperature_2m) || !Number.isFinite(data.daily?.temperature_2m_max?.[0]) || !Number.isFinite(data.daily?.temperature_2m_min?.[0]) || !Array.isArray(snow) || snow.length<6 || !snow.every(Number.isFinite))throw new Error('Mountain forecast unavailable: the source returned incomplete data.'); ctx.el.querySelector('.ski-temp').textContent = `${Math.round(data.current.temperature_2m)}°`; ctx.el.querySelector('.ski-data').innerHTML = `Today’s high: ${Math.round(data.daily.temperature_2m_max[0])}°<br>Today’s low: ${Math.round(data.daily.temperature_2m_min[0])}°<br>Snowfall forecast: ${snow[0].toFixed(1)} cm<br>Next 6 days: ${snow.slice(0,6).reduce((a, b) => a + b, 0).toFixed(1)} cm`; } catch (error) { if (ctx.alive()) { ctx.el.querySelector('.ski-temp').textContent='—°'; ctx.el.querySelector('.ski-data').textContent = error.message; } } }; load(); ctx.interval(load, 900000);
  ...
  register('movies', 'Movies', 270, 281, box('<span style="font:11px Georgia;background:#d2b3ed;color:#503866;border:3px double #85649e;padding:6px">ADMIT<br>ONE</span>', 'metal'), ctx => {
    ...
    let request = 0;
    ctx.el.querySelector('form').onsubmit = async event => { event.preventDefault(); const id = ++request, term = ctx.el.querySelector('input').value.trim(); if (!term) return; const results = ctx.el.querySelector('.movie-results'); results.textContent = 'Searching…'; try { const data = await fetchJSON(`https://itunes.apple.com/search?term=${encodeURIComponent(term)}&entity=movie&limit=12`); if (!ctx.alive() || request !== id) return; if(!Array.isArray(data.results))throw new Error('Movie results unavailable: the source returned incomplete data.'); results.innerHTML = data.results?.length ? data.results.map((movie, index) => `<button class="movie-result" data-index="${index}"><img src="${e(movie.artworkUrl100)}" alt=""><span>${e(movie.trackName)}<small>${e(movie.releaseDate?.slice(0, 4))} · ${e(movie.contentAdvisoryRating || '')}</small></span></button>`).join('') : '<div class="service-error">No movies found.</div>'; results.querySelectorAll('button').forEach(button => button.onclick = () => Dashboard.open(data.results[Number(button.dataset.index)].trackViewUrl)); } catch (error) { if (ctx.alive() && request === id) results.textContent = error.message; } };
